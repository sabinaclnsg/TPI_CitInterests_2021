...
if ($_SESSION['connected']) { // checks if a user is connected. if not, redirect to homepage
        ...
        if (!empty($sights_name) && !empty($description) && !empty($adress) && !empty($canton) && !empty($canton) && !empty($_FILES['image_file']) && !empty($sights_contains_category) && !empty($sights_has_age_limit)) {
            ...
            if ($price >= 1 || isset($_POST['price_free'])) { // checks if both price input and checkbox are empty
                ...
                if ($image_file['error'] === UPLOAD_ERR_OK) {
                    ...
                    try {
                        if (empty($price)) { // if price input is empty, automatically send "price" as NULL = free
                            $id_sights = SightsDAO::CreateSight($sights_name, NULL, $description, $adress, $canton, $img_name, $_SESSION['connected_user_id']);
                        } else {
