...
class UserDAO
{
    ...
    public static function UpdateUserIcon($id, $profile_icon)
    {
        try {
            $db = DBConnection::getConnection();
            $sql = "UPDATE `user` SET `profileImg` = :profle_icon WHERE `id` = :id";
            $request = $db->prepare($sql);
            $request->execute([':id' => $id, ':profle_icon' => $profile_icon]);
        } catch (mysqli_sql_exception $exception) {
            throw $exception;
        }
    }
